...
- ES: el early-skip de `kb-bootstrap` ahora verifica también `chunker_version`. Antes un bump del chunker no re-indexaba los procedures cuyo `last_altered` en Netezza no había cambiado — el auto-reindex del #11 quedaba huérfano para bootstrap.
- EN: the `kb-bootstrap` early-skip now checks `chunker_version` as well. Previously a chunker bump wouldn't re-index procedures whose Netezza `last_altered` hadn't changed — the auto-reindex guarantee from #11 was orphaned for bootstrap runs.
- ES: la barra de progreso ahora se pondera por `size_bytes` cuando nz-mcp lo expone. El ETA deja de oscilar cuando aparece una SP gigante porque Rich mide throughput en bytes/s en vez de procs/s. Fallback a count-based si `size_bytes` no está disponible.
- EN: progress bar is now weighted by `size_bytes` when nz-mcp provides it. ETA no longer swings when a huge SP appears because Rich measures throughput in bytes/s instead of procs/s. Falls back to count-based when `size_bytes` is unavailable.
